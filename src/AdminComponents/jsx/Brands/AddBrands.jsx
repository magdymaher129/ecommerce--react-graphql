...
import  Add_Brand  from "../../gql/brands/addbrand.js"
import Head from "../../panel/Head.jsx";
import SBar from "../../panel/SBar.jsx";
...
function AddBrands() {
  ...
  const OpenSidebar = () => {
    ...
    const {addNew,Change,data,result,info,setResult,setInfo,url}=useCat()
     ...
     const [createBrand] = useMutation(Add_Brand);
      ...
      const AddBrand = async (e) => {
       e.preventDefault();
        ...
        await createBrand({ variables: { title: item, image: id },
          onCompleted: () => {
            setResult("success");
      ...
      return (
        ...
        <div>
          ...
          <div className={styles.brandUpdateContainer}>
            <h2 className={styles.brandUpdateTitle} style={{color:"green",textTransform:"uppercase"}}>
             Add Brand
            </h2>
          ...
          {data ? (
            ...
            >
              ...
              <Button  variant="light"onClick={addNew} > new category</Button>
              </row>
            </form></div>
          </div>
        </div></div>
